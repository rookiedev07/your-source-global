  ...
  return (
    ...
    >
      <div
        ...
        className="relative w-full h-[100dvh] min-h-[600px] max-h-[960px] flex flex-col justify-between overflow-hidden"
      ...
      >
        ...
        <div className="relative z-20 pt-20 sm:pt-24 pb-2 border-b border-navy-800/80 bg-navy-950/80">
        ...
        <div className="relative z-10 flex-1 flex items-center py-4">
          <Container size="wide" className="w-full grid grid-cols-12 gap-6 lg:gap-8 items-center h-full max-h-[640px]">
            ...
            <div
              className="col-span-12 lg:col-span-8 relative h-[480px] sm:h-[520px] flex items-center"
              ...
              {SERVICES.map((service, idx) => {
                ...
                return (
                  <div
                    ...
                    className="absolute inset-0 w-full h-full p-6 sm:p-8 md:p-10 rounded-2xl sm:rounded-3xl bg-[#121E36] border border-navy-700/90 shadow-2xl flex flex-col justify-between"
                  >
                    <div>
                      <div className="flex items-center justify-between gap-3 mb-4 sm:mb-6 pb-3 sm:pb-4 border-b border-navy-800">
                        <div className="flex items-center gap-2.5 sm:gap-3">
                          <span className="text-3xl sm:text-4xl md:text-5xl font-display font-black text-slate-400/80 tracking-tight font-mono">
                            {service.number}
                          </span>
                          <div className="h-6 sm:h-8 w-px bg-navy-800" />
                          <div>
                            <span className="text-[10px] sm:text-xs font-mono uppercase tracking-widest text-slate-400 block font-bold">
                              Discipline {service.number} of 07
                            </span>
                            <span className="text-[11px] sm:text-xs text-amber-300 font-semibold">{service.tagline}</span>
                          </div>
                        </div>

                        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-navy-800 border border-navy-700 text-slate-200 flex items-center justify-center shadow-inner shrink-0">
                          <IconComponent className="w-5 h-5 sm:w-6 sm:h-6" />
                        </div>
                      </div>

                      <h3 className="text-xl sm:text-2xl md:text-3xl font-display font-bold text-white mb-2 sm:mb-3 tracking-tight">
                        {service.title}
                      </h3>
                      <p className="text-xs sm:text-sm md:text-base text-slate-300 leading-relaxed mb-4 sm:mb-6 line-clamp-3 sm:line-clamp-none">
                        {service.shortDescription}
                      </p>

                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-2.5 pt-3 sm:pt-4 border-t border-navy-800">
                        {service.deliverables.map((item, dIdx) => (
                          <div key={dIdx} className="flex items-start gap-2 text-xs text-slate-300">
                            ...
                            <span className="line-clamp-1 sm:line-clamp-none">{item}</span>
                          </div>
                        ))}
                      </div>
                    </div>

                    <div className="pt-4 sm:pt-6 border-t border-navy-800 flex flex-wrap items-center justify-between gap-3 sm:gap-4">
                      <div className="hidden sm:flex flex-wrap items-center gap-1.5 sm:gap-2">
                        <span className="text-[10px] sm:text-[11px] font-mono uppercase text-slate-400 font-semibold">
                          Ecosystem:
                        </span>
                        {service.tools.slice(0, 4).map((t, tIdx) => (
                          <span
                            key={tIdx}
                            className="text-[10px] sm:text-[11px] font-mono px-2 py-0.5 rounded bg-navy-800 border border-navy-700 text-slate-300"
                          >
                            {t}
                          </span>
                        ))}
                      ...
                      <Button
                        ...
                        size="sm"
                        icon={ArrowRight}
                        className="w-full sm:w-auto justify-center bg-white text-navy-950 font-bold hover:bg-slate-100 text-xs sm:text-sm"
                      >
                        Staff This Pod
                      </Button>
                    </div>
